...
i named the cleaned dataset as ‘brfss_df’. the cleaned dataset has 10625
rows and 23 cols.
...
- from the plot, we can find that the response variable (we switch it
  ...
  excellent, and here we treat good, vary good, and excellent as a
  cluster of response as good experience) increases when the data_value
  increase, That is, for observations with a higher data_value, the
  response will be higher correspondingly. And the density for response
  from lowest to highest is Good, Very good, excellent, Fair, and Poor
  in 2006 and Good, Very good, Fair, Excellent, Poor for 2010. The
  density of Poor in 2006 is less than that of 2010.
...
- 1st part code
  - I create a new dataframe called ‘final_easy_read_df’. here, i group
    ...
    group (name this variable as n_obs). Then i use `pivot_wider()` to
...
- 2nd part code
  - i create a histogram titled ‘Age distributions by sex and education
    category’ and treat age variable in x-axis and grouped by sex. Then
    i great 3 panels based on the category of education.
  - *comments*:
