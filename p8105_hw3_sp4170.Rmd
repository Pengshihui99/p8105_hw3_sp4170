...
i named the cleaned dataset as 'brfss_df'. the cleaned dataset has `r nrow(brfss_df)` rows and `r ncol(brfss_df)` cols.
...
final_df |> 
  ...
  geom_histogram(binwidth = 5, alpha = 0.3, position = 'dodge') +
...
* 1st part code
  * I create a new dataframe called 'final_easy_read_df'. here, i group the dataset by sex and education category using `group_by()` and then use `summarise()` to count how many observations in each single group (name this variable as n_obs). Then i use `pivot_wider()` to produce a reader-friendly table.
  * *comments*: 
...
* 2nd part code
  * i create a histogram titled 'Age distributions by sex and education category' and treat age variable in x-axis and grouped by sex. Then i great 3 panels based on the category of education.
  * *comments*: 
